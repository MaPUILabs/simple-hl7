    ...
    if (this.fields.length >= fieldIndex) {
        ...
        if (components.length >= componentIndex) {
            ...
            if (subComponentIndex) {
                if (typeof component.value[0] == 'string') {
                    return component.value[subComponentIndex - 1].toString(delimiters)
                } else {
...
segment.prototype.getComponentFromField = function (field, componentIndex, subComponentIndex) {
    if (field) {
        if (field.value) {
            var components = field.value[0];
            if (components.length >= componentIndex) {
            ...
            if (field.length >= componentIndex) {
                const component = field[componentIndex - 1];
                if (subComponentIndex) {
                    if (component.value[0].length >= subComponentIndex) {
                        return component.value[0][subComponentIndex - 1].toString(delimiters)
                    }
...
segment.prototype.removeComponent = function(fieldIndex, componentIndex) {
    if (this.fields.length >= fieldIndex) {
        var components = this.fields[fieldIndex - 1].value;
        if (components.length > 1) {
            for (let c of components)
                c.value[0].splice(componentIndex - 1, 1)
        } else {
            components[0].splice(componentIndex - 1, 1);
        }
    }
}
